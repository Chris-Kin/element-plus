...
export function createLoadingComponent({
  ...
  function destroySelf() {
    ...
    if (!target.vLoadingAddClassList) {
      ...
      if (!loadingNumber) {
        removeClass(target, 'NAMESPACE-loading-parent--relative')
        target.removeAttribute('loading-number')
      } else {
        target.setAttribute('loading-number', loadingNumber.toString())
      }
      removeClass(target, 'NAMESPACE-loading-parent--hidden')
    }
  ...
  const elLoadingComponent = {
    ...
    render() {
      ...
      const spinnerText = h('p', { class: 'NAMESPACE-loading-text' }, [this.text])

      return h(Transition, {
        name: 'NAMESPACE-loading-fade',
