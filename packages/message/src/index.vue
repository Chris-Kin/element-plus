<template>
  <transition name="NAMESPACE-message-fade" @before-leave="onClose" @after-leave="$emit('destroy')">
    <div
      v-show="visible"
      :id="id"
      :class="[
    ...
    >
      ...
      <slot>
        <p v-if="!dangerouslyUseHTMLString" class="NAMESPACE-message__content">{{ message }}</p>
        <!-- Caution here, message could've been compromised, never use user's input as message -->
        <!--  eslint-disable-next-line -->
        <p v-else class="NAMESPACE-message__content" v-html="message"></p>
      </slot>
      <div v-if="showClose" class="NAMESPACE-message__closeBtn NAMESPACE-icon-close" @click.stop="close"></div>
    </div>
  </transition>
</template>
<script lang="ts">
