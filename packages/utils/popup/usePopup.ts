...
const usePopup = (props: Readonly<Props>, doClose: () => void, rootRef = 'root') => {
  ...
  const doOpen = merProps => {
    ...
    if (modal) {
      ...
      if (merProps.lockScroll) {
        state.withoutHiddenClass = !hasClass(document.body, 'NAMESPACE-popup-parent--hidden')
        ...
        addClass(document.body, 'NAMESPACE-popup-parent--hidden')
      }
    }
  ...
  const restoreBodyStyle = () => {
    if (props.modal && state.withoutHiddenClass) {
      ...
      removeClass(document.body, 'NAMESPACE-popup-parent--hidden')
    }
