...
const newIndex = index.toString().replace(/@element-plus\//g, `./${G.NAMESPACE}-`).replace(`${G.NAMESPACE}-utils`, 'utils')
...
fs.readdirSync(libDirPath).forEach(comp => {
  if (!noElPrefixFile.test(comp)) {
    if (fs.lstatSync(path.resolve(libDirPath, comp)).isDirectory()) {
      // rename
      const newCompName = `${G.NAMESPACE}-${comp}`
      ...
      if(outsideImport.test(imp) || imp.includes('@element-plus/')) {
        const newImp = imp.replace(outsideImport, (i, c) => {
          return i.replace(`../${c}`, `../${G.NAMESPACE}-${c}`)
        }).replace('@element-plus/', `../${G.NAMESPACE}-`).replace(`${G.NAMESPACE}-utils`, 'utils')
        ...
        data.forEach(f => {
          if (!fs.lstatSync(path.resolve(srcPath, f)).isDirectory()) {
            ...
            if (imp.includes('@element-plus/')) {
              const newImp = imp.replace(/@element-plus\//g, '../../${G.NAMESPACE}-')
